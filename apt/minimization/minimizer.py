...
############An XOR encryption for HE####################
import struct

class XOREncryption:
    '''
    Using XOR to implement a simple Homomorphic encryption(HE)
    The dataset is encrypted by its datapoint type, i.e. string and float.
    Two keys are used to encrypted.
    ...
    data = np.array([[1.2, 'hello'], [3, 4.5]], dtype=object)
    ...
    '''
    def __init__(self, float_key=None, string_key=None):
        self.float_key = float_key
        self.string_key = string_key
        #self.scale_factor = 1e6
        self.precision = 5
        
    def float_to_fixed(self, floating_numbers):
        multiplier = 10 ** self.precision
        return np.int32(floating_numbers * multiplier)

    def fixed_to_float(self, fixed_numbers):
        multiplier = 10 ** self.precision
        return float(fixed_numbers / multiplier)

    # Encrption of fixed int data.
    # transform float data into fixed int data.
    # To encrypt and decrypt value, I impplement a judgement to do reverse process.
    def fixed_xor_encryption(self, val, key):
        #print(type(val),val)
        if isinstance(val, (float, np.float32, np.float64)):
            transformed_val = self.float_to_fixed(val)
            encrypted_val = transformed_val ^ key
        else:
            transformed_val = val ^ key
            encrypted_val = self.fixed_to_float(transformed_val)
        return encrypted_val
    
    
    '''
    # I implement a binary XOR encryption feature for flaot data.
    # But, the encrypted value is too large for dtype('float32').
    
    def float_to_bin(self, number):
        return ''.join(bin(c).replace('0b', '').rjust(8, '0') for c in struct.pack('!f', number))

    def bin_to_float(self, binary_str):
        return struct.unpack('!f', bytes(int(binary_str[i:i + 8], 2) for i in range(0, len(binary_str), 8)))[0]

    def float_xor_encryption(self, value, key):
        binary_val = self.float_to_bin(value)
        binary_key = self.float_to_bin(key)
        xor_result = ''.join(str(int(a) ^ int(b)) for a, b in zip(binary_val, binary_key))
        encrypted_val = self.bin_to_float(xor_result)
        return encrypted_val
    '''
    
    # string encryption function
    def string_xor_encryption(self, value, key):
        long_bits = max(len(value), len(key))
        key_extended = key * long_bits + key[:long_bits % len(key)]
        return ''.join([chr(ord(data_char) ^ ord(key_char)) for data_char, key_char in zip(value, key_extended)])
    
    # Main encryption function
    def encrypt(self, data):
        encrypted_data = np.empty_like(data, dtype=object)
        for idx, val in np.ndenumerate(data):
            if isinstance(val, (float, int, np.float32, np.float64, np.int32, np.int64)):
                encrypted_data[idx] = self.fixed_xor_encryption(val, self.float_key)
            else:
                val = str(val)
                encrypted_data[idx] = self.string_xor_encryption(val, self.string_key)
...
class GeneralizeToRepresentative(BaseEstimator, MetaEstimatorMixin, TransformerMixin):
    ...
    def fit(self, X: Optional[DATA_PANDAS_NUMPY_TYPE] = None, y: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        ...
        if self.estimator and dataset and dataset.get_samples() is not None and dataset.get_labels() is not None:
            ...
            # check accuracy
###########################################
# Change metric of classification. AUC is a better metric than accuracy

            if self.is_regression:
                accuracy = self.estimator.score(ArrayDataset(self.encoder.transform(generalized), y_test))
            else:
                y_pred = self.estimator.predict(ArrayDataset(self.encoder.transform(generalized)))
                
                accuracy = roc_auc_score(y_test,np.argmax(y_pred, axis=1),multi_class='ovr')
###########################################
